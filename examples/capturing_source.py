#! /usr/bin/python
"""Example meant to demonstrate capturing source code, i.e. html of the websites."""
...
file_handler = logging.FileHandler('capturing_source.log')
...
if __name__ == "__main__":
    logger.info('')
    logger.info('Starting "capturing_source" penguin client --->')
    logger.info('==========================================================')
    ...
    def driver_functionality(websites, chrome, timeouts):
        ...
        logger.debug('Chrome driving thread saved a screenshot and source for \'{}\''.format(name))
    ...
    def source_handler_functionality():
        try:
            ...
            logger.info('Source handler thread returned empty source directory.. closing thread')
            return False
        ...
        for current_file in queue:
            for __ in xrange(100000):
                try:
                    ...
                    logger.debug('Source handler thread processed \'{}\''.format(current_file))
                    break
                except OSError:
                    try:
